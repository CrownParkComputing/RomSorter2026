...
fn run_hash_test<R: Read + Seek>(
    ...
    for entry in &nca_entries {
        ...
        let sha = if entry.name.ends_with(".ncz") {
            ...
            if let Err(e) = decompress_ncz(reader, &mut tmp, nca_size, entry.abs_offset, entry.size, None)
            {
...
fn compute_restored_file_sha<R: Read + Seek>(
    ...
    if entry.name.ends_with(".ncz") {
        ...
        decompress_ncz(reader, &mut tmp, nca_size, entry.abs_offset, entry.size, None).ok()?;
