...
fn decompress_nsz(input_path: &str, output_path: &str) -> Result<()> {
    ...
    for entry in nsp.all_entries() {
        if entry.name.ends_with(".ncz") {
            ...
            let pb = progress::file_progress(
                nca_size,
                &format!("Decompressing {} ({} MB)", entry.name, entry.size / (1024 * 1024)),
            );
            let mut tmp = tempfile::NamedTempFile::new()?;
            ncz::decompress_ncz(&mut file, &mut tmp, nca_size, abs_offset, entry.size, Some(&pb))?;
            pb.finish_with_message(format!("Done {}", entry.name));
...
fn decompress_xcz(input_path: &str, output_path: &str) -> Result<()> {
    ...
    let total_ncz_bytes: u64 = secure
        ...
        .map(|e| e.size)
        .sum();
    let _ = total_ncz_bytes; // each NCZ gets its own bar below
    ...
    for entry in &secure.entries {
        ...
        if entry.name.ends_with(".ncz") {
            ...
            let nca_size = ncz_meta
                ...
                .max()
                .unwrap_or(entry.size);
            let decomp_pb = progress::file_progress(
                nca_size,
                &format!("Decompressing {} ({} MB)", entry.name, entry.size / (1024 * 1024)),
            );
            let mut tmp = tempfile::NamedTempFile::new()?;
            ncz::decompress_ncz(&mut file, &mut tmp, nca_size, abs_offset, entry.size, Some(&decomp_pb))?;
            decomp_pb.finish_with_message(format!("Done {}", entry.name));
    ...
    // individual per-NCZ bars finish themselves above
...
fn decompress_single_ncz(input_path: &str, output_path: &str) -> Result<()> {
    ...
    ncz::decompress_ncz(&mut file, &mut out, nca_size, 0, compressed_size, Some(&pb))?;
    out.flush()?;
    pb.finish_with_message("Done");
