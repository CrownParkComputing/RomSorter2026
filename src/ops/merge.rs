...
fn create_merge_temp_file(output_path: &str) -> Result<tempfile::NamedTempFile> {
    let output_parent = Path::new(output_path)
        .parent()
        .filter(|p| !p.as_os_str().is_empty());
    if let Some(parent) = output_parent {
        tempfile::NamedTempFile::new_in(parent).or_else(|_| tempfile::NamedTempFile::new())
    } else {
        tempfile::NamedTempFile::new()
    }
    .map_err(NscbError::from)
}
...
pub fn merge(
    ...
    progress::log(&format!(
        "Collecting NCA files from {} inputs...",
        input_paths.len()
    ));
    ...
    for path_str in input_paths {
        ...
        match ext.as_str() {
            "nsz" | "xcz" => {
                ...
                let tmp = create_merge_temp_file(output_path)?;
    ...
    if exclude_deltas {
        ...
        if removed > 0 {
            progress::log(&format!("Excluded {} delta fragment NCAs", removed));
        }
    }
    ...
    progress::log(&format!("Output written to {}", output_path));
    Ok(())
}
...
fn build_nsp_output(
    ...
    progress::log(&format!(
        "Building NSP with {} entries ({} bytes)...",
        items.len(),
        total
    ));
    ...
    for (idx, item) in items.iter().enumerate() {
        match item {
            ...
            Item::Ticket(tik) => {
                pb.set_message(format!("[{}/{}] {}", idx + 1, items.len(), tik.name));
            ...
            Item::Cert(cert) => {
                pb.set_message(format!("[{}/{}] {}", idx + 1, items.len(), cert.name));
            ...
            Item::Xml(xml) => {
                pb.set_message(format!("[{}/{}] {}", idx + 1, items.len(), xml.name));
...
fn build_xci_output(
    ...
    progress::log(&format!("Preparing XCI output from {} NCAs...", ncas.len()));
    ...
    for (idx, nca) in ncas.iter().enumerate() {
        if idx == 0 || idx + 1 == ncas.len() || idx % 10 == 0 {
            progress::log(&format!("Preparing XCI NCA {}/{}...", idx + 1, ncas.len()));
        }
        let mut src = BufReader::new(File::open(&nca.source_path).map_err(|e| {
            NscbError::Io(std::io::Error::new(
                e.kind(),
    ...
    progress::log(&format!(
        "Building XCI with {} secure entries ({} bytes)...",
        prepared.len(),
        total_data_size
    ));
    let mut out = BufWriter::new(File::create(output_path)?);
    ...
    // Write NCA data
    for (idx, nca) in prepared.iter().enumerate() {
        if idx == 0 || idx + 1 == prepared.len() || idx % 10 == 0 {
            progress::log(&format!(
                "Writing XCI entry {}/{}...",
                idx + 1,
                prepared.len()
